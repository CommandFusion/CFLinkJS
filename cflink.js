...
var CFLink = {
	 ...
	 * @private
	 */
	_createDevice: function(id, type, systemName) {
		switch (type) {
			case CFLink.model.LANBridge:
				return new CFLink.LANBridge(systemName, id, null);

			case CFLink.model.CFMini:
				return new CFLink.CFMini(systemName, id, null);

			case CFLink.model.SW16:
				return new CFLink.SW16(systemName, id, null);
			...
			case CFLink.model.DINMOD4:
				...
				return new CFLink.DINMOD4(systemName, id, null);

			case CFLink.model.IRBlaster:
				return new CFLink.IRBlaster(systemName, id, null);
		}
		return null;
	},
	 ...
	 */
	getDevice: function(systemName, deviceModel, cflinkID) {
		...
		dev = this._createDevice(cflinkID, deviceModel, systemName);
		if (dev != null) {
			registry[ident] = dev;	// add to registry using simple identity
		}
		return dev;
	},
		...
		for (var sn in this.deviceRegistry) {
			if (this.deviceRegistry.hasOwnProperty(sn)) {
				...
				if (dev.hasOwnProperty("modules")) {
					...
					for (i = 0; i < n; i++) {
						if (modules[i].ownsPort(portObject)) {
							return dev;
						}
					}
				}
	...
	/**
	 * Process incoming data forwarded to us by a LANBridge unit. Check if this
	 ...
	 * iViewer name of the external system through which we receive the data.
	 * @param {String} systemName	unused
	 * @param {String} data			the data to process
	 * @private
	 */
	incomingData: function (systemName, data) {
		...
		if (CFLink.interceptNetworkDialog != null) {
			CFLink.interceptNetworkDialog(true, systemName, data);
		}
		if (data.charAt(3) === 'R') {
			...
			if (CFLink.replyCallback !== null) {
				...
				// No callback defined, just log the incoming data
				//CF.log("No CFLink reply callback defined");
				//GUI.addLog(matches);
			}
		}
	...
	buildMsg: function (systemName, cflinkID, commandType, target, command, payload) {
		...
		var msg = "\xF2" + String.fromCharCode(parseInt(cflinkID, 16)) + "\xF3" + commandType + target + command + "\xF4" + payload + "\xF5\xF5";
		if (systemName != null) {
		...
		replyData:function (data, regex, me) {
			switch (data[4]) {
				...
				case "DID":
					...
					me.id = data[1].charCodeAt(0).toString(16);
...
CFLink.Module = Class.$extend(
	...
	watchIOPorts: function(portNumbers, callback, me) {
		...
		return CFLink.watch(CFLink.IOPort.VALUE_CHANGE, device, function (evt, sender, portObject, previousState, newState) {
			if (this.IOPorts.indexOf(portObject) != -1 && (portNumbers == null || portNumbers.indexOf(portObject.portNumber) != -1)) {
	...
	watchRelays : function(portNumbers, callback, me) {
		...
		if (!this._checkValidRelay(portNumbers, "watchRelays")) {
			return 0;
		}
		var device = CFLink.getDeviceForModulePort(this);
		return CFLink.watch(CFLink.IOPort.VALUE_CHANGE, device, function (evt, sender, portObject, previousState, newState) {
			if (this.Relays.indexOf(portObject) != -1 && (portNumbers == null || portNumbers.indexOf(portObject.portNumber) != -1)) {
				callback.apply(me, [device, this, portObject.portNumber, previousState, newState]);
			}
		}, this);
	},
	...
	configureCOMPort : function(index, mode, baud, dataBits, parity, stopBits, flowControl) {
		this._sendPortCommand("IOX", "SPC", index, "configureCommand", [ mode, baud, dataBits, parity, stopBits, flowControl ], "C");
	},
	...
	watchCOMPorts: function(portNumbers, callback, me) {
		...
		return CFLink.watch(CFLink.RS232Port.SERIAL_DATA_RECEIVED, device, function (evt, sender, portObject, data) {
			if (this.COMPorts.indexOf(portObject) != -1 && (portNumbers == null || portNumbers.indexOf(portObject.portNumber) != -1)) {
 ...
 *
 * function(event, sender, LEDobject)
 *
...
CFLink.RelayPort = CFLink.IOPort.$extend(
	...
	setCommand : function(open) {
		return this.portNumberString + (open ? ":1" : ":0");
	},
...
CFLink.RS232Port = Class.$extend(
	...
	__init__ : function (systemName, deviceID, cflinkPrefix, portNum) {
		...
		this.mode = (portNum == null) ? "232" : "PGM";	// default mode, RS232 for COM port and CFLink programming for onboard port
		this.baud = 115200;
		this.dataBits = 8;
		this.stopBits = 1;
		this.parity = "N";
...
CFLink.LANBridge = CFLink.Device.$extend(
	 ...
	 */
	{
		...
		__init__:function (systemName, cflinkID, dataString) {
			...
			for (i = 3; i <= 20; i++) {
				this.slots[i] = new CFLink.LANBridge.Slot(systemName, cflinkID, "LAN", i + 1, false);
			}
 ...
 */
{
	...
	__init__: function(systemName, id, dataString) {
		// onboard RS232 port
		this.rs232Port = new CFLink.RS232Port(systemName, id);
...
CFLink.CFMini = CFLink.Device.$extend(
	...
	__init__ : function (systemName, id, dataString) {
        ...
        // Listen for replies from IRBlaster units on this ID
        CFLink.attachReplyCallbackByCFLinkID(id, this.replyData, this);
	...
	watchRelays : function(portNumbers, callback, me) {
		...
		return CFLink.watch(CFLink.IOPort.VALUE_CHANGE, this, function (evt, sender, portObject, previousState, newState) {
			if ((portObject instanceof CFLink.RelayPort) && (portNumbers == null || portNumbers.indexOf(portObject.portNumber) != -1)) {
...
CFLink.SW16 = CFLink.Device.$extend(
	 ...
	 */
	{
		 ...
		 * @param index        a single led number (1-16) or an array of led numbers (i.e. [1,3,5,9])
		 * @param on        true to turn on, false to turn offevel of the LED, 0-100 (can also pass 1 / 0)
		 */
